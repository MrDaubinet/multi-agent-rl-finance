'''
	Strategy 1:
		Environment: Stock - NFLX
			...
			Reward Strategy: proportional-short-net-worth-change
		DRL: 
			PPO:
...
from rl_fts.strategies.stock.NFLX.environment3.models.model1 import KerasBatchNormModel
# Environment
from rl_fts.environments.stock.NFLX.environment3 import create_env, normalization_info, reward_clipping
from rl_fts.environments.stock.NFLX.environment3 import max_episode_reward_training, max_episode_reward_evaluation
from rl_fts.environments.stock.NFLX.environment3 import training_length, evaluation_length
# Callbacks
...
class PPO_NFLX_PBSH_NWC(Strategy):
	...
	def __init__(self):
			...
			self.log_name = "stock/NFLX/environment3/strategy1"
			...
			config = (
				...
				[
					RecordTotalRewardCallback, 
					RecordNetWorthCallback, 
					RecordTotalTradesCallback
				]))
			)
